...
func _ready():
	...
	set_root_address(TranslationServer.translate("FORUM_NAME"), "res://Scenes/ForumlMenu.tscn")
	...
	for id: String in address:
		...
		##如果该页面ID位于现在选择的页面ID后面，则标记为要清除的页面
			...
			if last_selected_id == -1:
				last_selected_id = id.to_int()
				last_scene_node = pages.get_node(id)
			address_tab_bar.remove_tab(id.to_int())
			address.erase(id)
			address_size -= 1
			queue_free_nodes.append(pages.get_node(id))
